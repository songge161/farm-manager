      ...
      <view class="body">
        <view class="item">
          ...
          <view class="image-grid">
            <image
              v-for="(img, imgIndex) in record.methods[0].images"
              :key="imgIndex"
              :src="img"
              mode="aspectFill"
              @click="previewImage(img, record.methods[0].images)"
            ></image>
          </view>
        ...
        <view
          ...
          v-if="!expandedStates.get(record.id) && getArrayLength(record.methods)  > 1"
        >
          <view class="expand-button" @click="toggleExpand(record.id)">
            <text class="expand-text"
              >还有{{ getArrayLength(record.methods)  - 1 }}条操作记录</text
            >
        ...
        <view class="expanded-content" v-if="expandedStates.get(record.id)">
          <view
            v-for="(method, mIndex) in record.methods.slice(1)"
            :key="mIndex"
            class="item"
          >
            <view class="operation-info">
              <view class="main-info">
                <text class="action-type">{{ 
                  ...
                  method.title}}</text>
                <text class="time">{{ method.time }}</text>
              </view>
              <view class="weather-info">
                <text class="weather"
                  >{{ method.weather }} · {{ method.temperature }}</text
                >
              </view>
            </view>
            <view class="details">
            ...
            <view class="image-grid">
              <image
                v-for="(img, imgIndex) in method.images"
                :key="imgIndex"
                :src="img"
                mode="aspectFill"
                @click="previewImage(img, method.images)"
              ></image>
            </view>
            <view class="creator">
              <text class="creator-label">记录员：</text>
